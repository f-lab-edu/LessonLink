...
@router.delete("/", status_code=204, tags=["Bucket"])
def delete_bucket_by_id_handler(
    ...
    exists = redis_client.sismember(f"user:student:{request.id}:bucket", request.course_id)

    if not exists:
        raise HTTPException(status_code=404, detail="Not Found.")
    
    redis_client.srem(f"user:student:{request.id}:bucket", request.course_id)